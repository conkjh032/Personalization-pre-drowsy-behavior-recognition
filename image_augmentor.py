...
class ImageAugmentor:
    ...
    def get_random_transform(self, images):
        ...
        pairs_of_images = [np.zeros(
            (number_of_pairs_of_images * 5, self.image_height, self.image_height, 3)) for i in range(2)]

        for pair_index in range(number_of_pairs_of_images):
            image_1 = images[0][pair_index, :, :, :]
            image_2 = images[1][pair_index, :, :, :]

            pairs_of_images[0][pair_index * 5, :, :, :] = image_1
            pairs_of_images[1][pair_index * 5, :, :, :] = image_2

            if random_numbers[pair_index * 2, 0] > 0.5:
                pairs_of_images[0][pair_index * 5 + 1, :, :, :] = self._perform_random_rotation(image_1)
            if random_numbers[pair_index * 2, 1] > 0.5:
                pairs_of_images[0][pair_index * 5 + 2, :, :, :] = self._perform_random_shear(image_1)
            if random_numbers[pair_index * 2, 2] > 0.5:
                pairs_of_images[0][pair_index * 5 + 3, :, :, :] = self._perform_random_shift(image_1)
            if random_numbers[pair_index * 2, 3] > 0.5:
                pairs_of_images[0][pair_index * 5 + 4, :, :, :] = self._perform_random_zoom(image_1)

            if random_numbers[pair_index * 2 + 1, 0] > 0.5:
                pairs_of_images[1][pair_index * 5 + 1, :, :, :] = self._perform_random_rotation(image_2)
            if random_numbers[pair_index * 2 + 1, 1] > 0.5:
                pairs_of_images[1][pair_index * 5 + 2, :, :, :] = self._perform_random_shear(image_2)
            if random_numbers[pair_index * 2 + 1, 2] > 0.5:
                pairs_of_images[1][pair_index * 5 + 3, :, :, :] = self._perform_random_shift(image_2)
            if random_numbers[pair_index * 2 + 1, 3] > 0.5:
                pairs_of_images[1][pair_index * 5 + 4, :, :, :] = self._perform_random_zoom(image_2)
